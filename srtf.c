...
void main()
{
	...
	while(1)
	{
		...
		for(j = 0; j < n; j++)
		{
			...
			{
				arr[j].time_remaining = arr[j].time_remaining - 1;

				if(!arr[j].time_remaining)
				{
					arr[j].completion_time = time + 1;
					arr[j].waiting_time = time - arr[j].arrival_time - arr[j].burst_time + 1;
