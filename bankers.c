...
void res_request(int allocation[10][10], int need[10][10], int work[10], int pid, int m)
{
	int reqmat[10];
	int i;
	...
	for(i = 0; i < m; i++)
	{
		if(reqmat[i] > work[i] || reqmat[i] > need[pid][i])
		{
...
void main(){
	...
	int work[10];
	int n,m,pid,ch;
	accept(allocation,need,max,work,&n,&m);
	ret = banker(allocation,need,work,n,m);
	if(ret !=0 )
	{
		printf("Do you want make an additional request ? (1.Yes|0.No)\n");
		scanf("%d",&ch);
		if(ch == 1)
		{
			printf("Enter process no. : \n");
			scanf("%d",&pid);
			res_request(allocation, need , work, pid, m);
			ret = banker(allocation ,need, work , n ,m);
			if(ret == 0 )
				exit(0);
		}
